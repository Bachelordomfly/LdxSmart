...
class getCargo:
    ...
    #   选择货物类型
    def cargotype_select(self, driver, index):
        try:
    ...
    def freight_caculate(self, waybill):
        ...
        if not freight_config:
            ...
            if not ori_weight_num:
                for ori_weight in self.freightInfo.find({'productCustomer_id': product_customer_id, 'cargo_type': cargo_type}):
                    ori.append(ori_weight['ori_weight'])

                for des_weight in self.freightInfo.find({'productCustomer_id': product_customer_id, 'cargo_type': cargo_type}):
    ...
    # 计算燃油附加费
    def fuel_surcharge(self, waybill):
        # 判断该站点是否有运费的费用配置
        site_id = self.packageItem.find_one({'waybill_no': waybill})['current_site']
        fuel_surcharge_config = self.chargeConfig.find_one({'site_id': site_id, 'charge_item': "燃油附加费"})
        # 获取当前燃油附加费
        waybill_id = self.packageItem.find_one({'waybill_no': waybill})['_id']
        account_id = self.accountInfo.find_one({'waybill_id': waybill_id})['_id']
        fuel_surcharge = self.charging.find_one({'account_id': account_id, 'item': '燃油附加费'})['fee']
        correct_fuel_surcharge = 0

        if not fuel_surcharge_config:
            customer_id = self.packageItem.find_one({'waybill_no': waybill})['customer_id']
            product_id = self.packageItem.find_one({'waybill_no': waybill})['product_id']
            try:
                #  产品客户有关联,取产品客户关联id
            ...
            # 获取正确燃油附加费
            try:
                fuel_surcharge_ratio = self.productCustomer.find_one({'_id': product_customer_id})['fuel_surcharge_ratio']  # 获取燃油费率
                if not fuel_surcharge:
                    freight = self.charging.find_one({'account_id': account_id, 'item': '运费'})['fee']  # 获取当前运费
                    correct_fuel_surcharge = freight * fuel_surcharge_ratio
